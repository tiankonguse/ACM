...
bool test_eq(A ans, B expectAns) {
  auto ExpectTypeAns = ans;
  ExpectTypeAns = expectAns;
  return eq(ans, ExpectTypeAns);
}

#define TEST_INDEX_OUTPUT(name) \
  printf("index[%d] %d: %s\n", __LINE__, getIndex(), name);
#define TEST_ANS_OUTPUT(ans, expectAns) \
  do {                                  \
    output("ans", ans);                 \
    output("expectAns", expectAns);     \
  } while (0)

#define TEST_PARAM_INIT1(p1) auto pp1 = p1
#define TEST_PARAM_OUTPUT1(p1) output("firstData", p1)

#define TEST_SMP1(ClassName, FunNAme, expectAns, p1) \
  do {                                               \
    ClassName work;                                  \
    TEST_PARAM_INIT1(p1);                            \
    auto ans = work.FunNAme(pp1);                    \
    bool check = test_eq(ans, expectAns);            \
    if (!check) {                                    \
      TEST_INDEX_OUTPUT("NO");                       \
      TEST_PARAM_OUTPUT1(p1);                        \
      TEST_ANS_OUTPUT(ans, expectAns);               \
    } else {                                         \
      TEST_INDEX_OUTPUT("YES");                      \
    }                                                \
  } while (0)

#define TEST_PARAM_INIT2(p1, p2) \
  TEST_PARAM_INIT1(p1);          \
  auto pp2 = p2

#define TEST_PARAM_OUTPUT2(p1, p2) \
  TEST_PARAM_OUTPUT1(p1);          \
  output("secondData", p2)

#define TEST_SMP2(ClassName, FunNAme, expectAns, p1, p2) \
  do {                                                   \
    ClassName work;                                      \
    TEST_PARAM_INIT2(p1, p2);                            \
    auto ans = work.FunNAme(pp1, pp2);                   \
    bool check = test_eq(ans, expectAns);                \
    if (!check) {                                        \
      TEST_INDEX_OUTPUT("NO");                           \
      TEST_PARAM_OUTPUT2(p1, p2);                        \
      TEST_ANS_OUTPUT(ans, expectAns);                   \
    } else {                                             \
      TEST_INDEX_OUTPUT("YES");                          \
    }                                                    \
...
#define TEST_PARAM_INIT3(p1, p2, p3) \
  TEST_PARAM_INIT2(p1, p2);          \
  auto pp3 = p3

#define TEST_PARAM_OUTPUT3(p1, p2, p3) \
  TEST_PARAM_OUTPUT2(p1, p2);          \
  output("thirdData", p3)

#define TEST_SMP3(ClassName, FunNAme, expectAns, p1, p2, p3) \
  do {                                                       \
    ClassName work;                                          \
    TEST_PARAM_INIT3(p1, p2, p3);                            \
    auto ans = work.FunNAme(pp1, pp2, pp3);                  \
    bool check = test_eq(ans, expectAns);                    \
    if (!check) {                                            \
      TEST_INDEX_OUTPUT("NO");                               \
      TEST_PARAM_OUTPUT3(p1, p2, p3);                        \
      TEST_ANS_OUTPUT(ans, expectAns);                       \
    } else {                                                 \
      TEST_INDEX_OUTPUT("YES");                              \
    }                                                        \
...
#define TEST_PARAM_INIT4(p1, p2, p3, p4) \
  TEST_PARAM_INIT3(p1, p2, p3);          \
  auto pp4 = p4

#define TEST_PARAM_OUTPUT4(p1, p2, p3, p4) \
  TEST_PARAM_OUTPUT3(p1, p2, p3);          \
  output("fouthData", p4)

#define TEST_PARAM_CALL4() TEST_PARAM_CALL3(), tmpFouthData

#define TEST_SMP4(ClassName, FunNAme, expectAns, p1, p2, p3, p4) \
  do {                                                           \
    ClassName work;                                              \
    TEST_PARAM_INIT4(p1, p2, p3, p4);                            \
    auto ans = work.FunNAme(pp1, pp2, pp3, pp4);                 \
    bool check = test_eq(ans, expectAns);                        \
    if (!check) {                                                \
      TEST_INDEX_OUTPUT("NO");                                   \
      TEST_PARAM_OUTPUT4(p1, p2, p3, p4);                        \
      TEST_ANS_OUTPUT(ans, expectAns);                           \
    } else {                                                     \
      printf("index %d: YES\n", getIndex());                     \
    }                                                            \
