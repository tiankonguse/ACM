...
int A[N], n; LL F[2][N]; bool C[2][N];

LL f(int s, int i){
    if (i <= 0 || i > n) return 0;
    LL &res = F[s][i]; if (!res){
        if (C[s][i] == 1) return -INFF; C[s][i] = 1;
        if (!s) res = A[i] + f(s^1, i + A[i]);
        else res = A[i] + f(s^1, i - A[i]);
    }
    return res;
}
...
int main(){
    ...
    RD(n); FOR_1(i, 2, n) RD(A[i]);
    RST(F, C); C[0][1] = 1; FOR(i, 1, n){
        F[1][1] = 0, A[1] = i; OT(max(f(1, 1+i)+i, -1LL));
    }
}
