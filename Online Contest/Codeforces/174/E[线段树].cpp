...
VII op[N]; int A[N], n, m;

int L[4*N], R[4*N], bj[4*N];
int a, b; LL res; int cnt;
...
void Query_l(int x, int l, int r){
    if (r < a) cnt += L[x];
    else {
        release(x);
        Query_l(lc);
        if (mid < a-1) Query_l(rc);
    }
}

void Query_r(int x, int l, int r){
    if (a < l) cnt += R[x];
    else {
        release(x);
        if (a+1 <= mid) Query_r(lc);
        Query_r(rc);
    }
}
...
int main(){
    ...
    RD(n, m); REP(i, n) RD(A[i]); sort(A, A + n); DO(m){
        a = lower_bound(A, A+n, RD()) - A;
        b = upper_bound(A, A+n, RD()) - A - 1;
        if (a >= b) continue;
        op[a].PB(MP(a, b)), op[b+1].PB(MP(a, b));
    }

    Build(root); res = (LL) n * (n - 1) * (n - 2) / 3;
    ...
    REP(i, n){
        ...
        cnt = 0, a = i; if (i) Query_l(root); if (i != n-1) Query_r(root);
        res -= (LL) cnt * (cnt - 1);
    }
