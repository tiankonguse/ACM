...
namespace ACM{ // Aho–Corasick Automator
    const int Z = 26, L = 109;
    int trans[N][Z], fail[N], Q[N], u, cz, op, tot, nn;
    int virus[N], mask[N], val[N]; // 病毒串？。。记号？。。得分？。。
    char str[L];
} using namespace ACM;

inline int new_node(){
    fail[tot] = virus[tot] = mask[tot] = val[tot] = 0, RST(trans[tot]);
    return tot++;
}
#define v trans[u][c]
...
inline void Build(){
    ...
    while (cz < op){
        u = Q[cz++]; REP(c, Z)
            if (v) fail[Q[op++] = v] = f, virus[v] |= virus[f], mask[v] |= mask[f], val[v] += val[f]; // .. .
            else v = f;
    }
}
#define c (*cur - 'a')
inline void Insert(){
    ...
    int x; RD(x);
    if (x == 999) mask[u] |= _1(nn++);
    else if (x == -999) virus[u] |= 1;
    else val[u] += x;
}
#define vis Q
#undef f

int f[2][N][1<<8], g[2][N][1<<8];
// 删了几个。。得分。。
...
inline int Run(){
    ...
    int p = 0, q = 1; FLC(f[p], 0x3f), FLC(g[p], 0x8f);
    f[0][0][0] = 0, g[0][0][0] = 0;
...
#define ss (s|mask[v])
    ...
    REP_S(cur, str){
        ...
        swap(p, q); FLC(f[p], 0x3f), FLC(g[p], 0x8f);
        REP(u, tot) REP(s, _1(nn)){

            if (!virus[v]){
                if (f[q][u][s] < f[p][v][ss] || f[q][u][s] == f[p][v][ss] && g[q][u][s] + val[v] > g[p][v][ss]){
                    f[p][v][ss] = f[q][u][s];
                    g[p][v][ss] = g[q][u][s] + val[v];
                }
            }

            if (f[q][u][s] + 1 < f[p][u][s] || f[q][u][s] + 1 == f[p][u][s] && g[q][u][s] > g[p][u][s]){
                f[p][u][s] = f[q][u][s] + 1;
                g[p][u][s] = g[q][u][s];
            }
        }
    ...
    int ff = INF, gg, s = _U(nn); REP(u, tot){
        if (f[p][u][s] < ff || f[p][u][s] == ff && g[p][u][s] > gg)
            ff = f[p][u][s], gg = g[p][u][s];
    }

    if (ff == INF) puts("Banned");
    else printf("%d %d\n", ff, gg);
}
