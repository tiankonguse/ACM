...
void Dijkstra(){
    ...
    nn = n*n, s = nn, t = (n-1)*n; REP(i, nn) CLR(adj[i]);

    int a, b; edge x; REP_2(i, j, n, n) if (G[i][j]){
        int h2 =  lower_bound(ALL(P), H[j]) - P.begin();
        REP(h1, n) REP(h2, h1+1){
            a = i*n+h1, b = j*n+h2; x.w = abs(H[j] - P[h2]);
            x.p = b; adj[a].push_back(x);
        }
    }

    FLC(D, 0x3f), FLC(C, true); while (!Q.empty()) Q.pop();
    ...
    ans = -1; int u, v; DO(nn){
        ...
        if (u >= t){ans = D[u]; return;}

        C[u] = false; ECH(it, adj[u])
            relax(u, it->p,  it->w);
    }
}
...
public:
	long long minCost(vector <string> G, vector <int> H) {
	    ::H = H; n = SZ(H); CLR(P); REP(i, n) P.PB(H[i]); SRT(P);
        REP_2(i, j, n, n) ::G[i][j] = G[i][j] == 'Y'; Dijkstra();
        return ans;
	}
};
