/** Micro Mezzo Macro Flation -- Overheated Economy ., Last Update: Jan. 15th 2013 **/ //{
...
const int N = 259;
Po C[N], P[N]; Line L[N][N]; LL _[3], *mask = _+1; bool del[N][N];
int A[N][N], F[N][N], G[2][N][N];
...
public:
	int countWays(int n, int r, vector <int> x, vector <int> y, string c) {
		...
		int m = SZ(x); REP(i, n) C[i] = Po(cos(2*PI/n*i), sin(2*PI/n*i));
		REP(i, m) P[i] = Po(x[i], y[i]) / r;

#define Lij L[i][j]
		RST(A); REP(i, n) FOR(j, i+1, n){
		    Lij = Line(C[i], C[j]); mask[-1] = mask[1] = 0;REP(k, m){
		        int t = Lij.side(P[k]); mask[t] |= _1(h(c[k]));
		        if (t==-1) ++A[i][j];
		    }
		    if (i+1==j && A[i][j]) return 0;
		    del[i][j] = mask[-1]&mask[1];
		}

		if (A[0][n-1] != m) return 0;
...
namespace moj_harness {
	...
	int run_test_case(int casenum) {
		...
		case 0: {
			...
			int received__            = FencingPenguins().countWays(numPosts, radius, vector <int>(x, x + (sizeof x / sizeof x[0])), vector <int>(y, y + (sizeof y / sizeof y[0])), color);
