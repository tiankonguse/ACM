...
int get_max(){
    int x = -INF, ii;
    REP(i, n) if (!C[i] && A[i] > x) x = A[i], ii = i;
    C[ii] = 1;
    return x;
}
...
int get_min(int l){
    int x = INF, ii;
    REP(i, n) if (!C[i] && A[i] < x && A[i] >= l) x = A[i], ii = i;
...
class TeamContest {
public:
	int worstRank(vector <int> _A) {
	    A = _A;

		n = SZ(A); xx = max(A[0], A[1], A[2])  + min(A[0], A[1], A[2]);

		A[0] = A[1] = A[2] = -INF; SRT(A);

		RST(C), C[0] = C[1] = C[2] = 1; error = 0;

		int res = 1; DO((n/3)-1) if (gao() > xx) ++res;
