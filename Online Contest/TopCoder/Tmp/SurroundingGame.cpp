...
const int N = 2*50*50 + 9, M = 10*N + 9;
...
public:
	int maxScore(vector <string> cost, vector <string> benefit) {
	    ...
	    r = SZ(cost), c = SZ(cost[0]), rc = r*c;
	    Sum = 0, t = rc*2+1, n = t+1, m = 2, fill(hd, hd+n, 0);

	    REP_2(i, j, r, c){

	        int net_benefit = get(benefit[i][j]) - get(cost[i][j]);
            Sum += get(benefit[i][j]) + max(net_benefit, 0);
            add_edge(id(i, j), idd(i, j), INF);

	        if (i&1^j&1){
                if (net_benefit > 0){
                    add_edge(s, id(i, j), net_benefit);
                }
                else {
                    add_edge(id(i, j), t, -net_benefit);
                }

                REP(d, 4){
                    int x = i + dx[d], y = j + dy[d];
                    if (inGrid(x, y)) add_edge(idd(x, y), idd(i, j), INF);
                }
                add_edge(idd(i, j), t, get(benefit[i][j]));
	        }
	        else {
	            if (net_benefit > 0){
	                add_edge(idd(i, j), t, net_benefit);
	            }
	            else {
	                add_edge(s, idd(i, j), -net_benefit);
	            }

                REP(d, 4){
                    int x = i + dx[d], y = j + dy[d];
                    if (inGrid(x, y)) add_edge(id(i, j), id(x, y), INF);
                }
                add_edge(s, id(i, j), get(benefit[i][j]));
	        }
	    }
