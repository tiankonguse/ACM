...
const int N = 50, M = int(1e5) + 9;
int n, m; Int Cm[M];

Int f(VI& a){
    Int res = 0; for (int o = a[0]-m; o <= a[0]+m; o += 2){
        Int cur = 1; REP(i, n){
            int d = m - abs(o - a[i]);
            if (d&1||d<0||2*m<d){
                cur = 0;
...
public:
	int calc(vector <int> x, vector <int> y, int m) {
	    ::m = m, Cm[0] = 1; REP_1(i, m) Cm[i] = Cm[i-1] * Int(m - i + 1) / Int(i);
        n = SZ(x); VI a(n); Int res = 1;
        REP(i, n) a[i] = x[i] - y[i]; res *= f(a);
        REP(i, n) a[i] = x[i] + y[i]; res *= f(a);
		return res;
	}
};
