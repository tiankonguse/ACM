...
void Ocaml(string file = ""){Exec("Ocamlc", file, "ml"); system("camlprog");}
...
int main(){
    ...
    dp[0][0][6] = 1; REP(i, n) REP(j, i+1) REP(k, 8) if(dp[i][j][k]){
        int kk = (k+k) % 8; dp[i+1][j][kk] += dp[i][j][k];
        if(!(k&2)) dp[i+1][j+1][kk|4] += dp[i][j][k];
        if(i + 1 < n) dp[i+1][j+1][kk|1] += dp[i][j][k];
    }

    REP(i, n+1){
        REP(j, 8) F[i] += dp[n][i][j];
        REP_1(j, n-i) F[i] = F[i] * Int(j);
    }

    DWN(i, n+1, 0) FOR_1(j, i+1, n) F[i] -= F[j] * C[j][i];
    OT(F[k]);
}
