...
struct grid{
    int c[4]; bool _;
    void input(){
        REP(i, 4) c[i] = color(RC());
        _ = RD();
    }
} G[9];

int A[3][3], step;
bool vis_c[4], vis[3][3][4];

bool inGrid(int x, int y){
    return 0 <= x && x < 3 && 0 <= y && y < 3;
}

void dfs(int x, int y, int o, int cc){
    if (vis[x][y][o]) return; vis[x][y][o] = 1;
    REP(i, 4) if (i != o && G[A[x][y]].c[i] == cc) dfs(x, y, i, cc);
    int xx = x + dx[o], yy = y + dy[o];
    if (inGrid(xx, yy) && G[A[xx][yy]].c[o^1] == cc) dfs(xx, yy, o^1, cc);
}


bool flood_fill(){

    RST(vis_c, vis); REP(i, 3) REP(j, 3) REP(o, 4) if (!vis[i][j][o]){
        if (vis_c[G[A[i][j]].c[o]]) return 0;
        else{
            dfs(i, j, o, G[A[i][j]].c[o]);
            vis_c[G[A[i][j]].c[o]] = 1;
        }
    }
...
bool dfs(int k = 0){
    if (k == step) return flood_fill();

    REP(i, SZ(op)){
        op[i].p(); if (dfs(k+1)) return 1; op[i].q();
        op[i].q(); if (dfs(k+1)) return 1; op[i].p();
    }
...
void reset(){
    int t = 0; REP_2(i, j, 3, 3) A[i][j] = t++;

    CLR(op);
    if (!G[0]._ && !G[1]._ && !G[2]._) op.PB(Op(0, 0));
    if (!G[3]._ && !G[4]._ && !G[5]._) op.PB(Op(1, 0));
    if (!G[6]._ && !G[7]._ && !G[8]._) op.PB(Op(2, 0));

    if (!G[0]._ && !G[3]._ && !G[6]._) op.PB(Op(0, 1));
    if (!G[1]._ && !G[4]._ && !G[7]._) op.PB(Op(1, 1));
    if (!G[2]._ && !G[5]._ && !G[8]._) op.PB(Op(2, 1));
}
...
int main(){
    ...
    Rush{
        REP(i, 9) G[i].input(); reset(); step = 0;
        while (!dfs()) ++step; OT(step);
    }
}
