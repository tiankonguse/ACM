...
int h[N], a[N], b[N]; MSI A, B; int an, bn; string s;
// height, preferred music style, favourite sport .. .
VI adj[N]; bool vis[N]; int p[N]; VI X, Y;
// graph .. .
int n;

#define y (*it)
bool dfs(int x){
    ECH(it, adj[x]) if (!vis[y]){
        vis[y] = true;
        if (p[y] == -1 || dfs(p[y])){
            p[y] = x;
            return true;
...
int main(){
    ...
    Rush{
        CLR(X, Y, A, B); an = bn = 0; REP_C(i, RD(n)){
            RD(h[i]); if (RC() == 'M') X.PB(i); else Y.PB(i);
            cin >> s; if (!A[s]) A[s] = ++an; a[i] = A[s];
            cin >> s; if (!B[s]) B[s] = ++bn; b[i] = B[s];
        }

#define x X[i]
#define y Y[j]

        REP(i, SZ(X)) CLR(adj[i]); REP_2(i, j, SZ(X), SZ(Y)) if (abs(h[x] - h[y]) <= 40 && a[x] == a[y] && b[x] != b[y]){
            adj[i].PB(j);
        }

        int res = n; FLC(p, -1); REP(i, SZ(X)){
            RST(vis); if (dfs(i)) --res;
        }
