...
bool bfs(){
    ...
    RST(D), D[Q[0] = s] = 1; int u; while (cz < op){
        u = Q[cz++]; REP(i, SZ(adj[u])) if (!D[v] && C[u][v]){
            Q[op++] = v, D[v] = D[u] + 1;
            if (v == t) return 1;
        }
    }
...
int Dinitz(){
    int max_flow = 0; while (bfs()){
        ...
        REP(i, n) cur[i] = 0; sta[0] = s;

        int u, v; while (top != -1){

            u = sta[top]; if (u == t){
                int delta = INF, handle; REP(i, top){
                    u = sta[i], v = sta[i+1];
                    if (C[u][v] < delta){
                        delta = C[u][v];
                        handle = i;
...
#define v adj[u][i]
            ...
            int i; FOR_N(i, cur[u], SZ(adj[u]))
                if (D[u] + 1 == D[v] && C[u][v]) break;

            if (i == SZ(adj[u])) D[u] = 0, --top;
            else {
                cur[u] = i + 1;
                sta[++top] = v;
            }
...
void init(){
    RD(n, m, k); REP(i, k) RD(st[i]);
    t=1, ++n; REP(i, n) CLR(adj[i]);
    ...
    REP(i, k) adj[s].PB(st[i]);
}

int solve(){
    int l = 0, r = k; while (l < r){
        int mid = l + r >> 1;
        REP(u, n) REP(i, SZ(adj[u])) C[u][v] = 0;
        REP(i, k) add_edge(s, st[i], 1);
        FOR(u, 1, n) REP(i, SZ(adj[u])) add_edge(u, v, mid);
        if (Dinitz() == k) r = mid; else l = mid + 1;
    }
    return l;
}
