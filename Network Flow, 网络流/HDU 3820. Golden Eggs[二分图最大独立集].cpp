...
const int N = 2 * (50*50) + 9, M = 2 * (3*N+4*N) + 9;
...
inline int id(int x, int y, bool t = 0){
    return x*c+y+1 + (t?rc:0);
}
...
void init(){
    RD(r, c), rc= r*c;
    t = 2*r*c+1, n = t+1, m = 2;
    Sum = 0, fill(hd, hd+n, 0);

    int G, S; RD(G, S);

    REP_2(i, j, r, c) add_edge(id(i, j), id(i, j, 1), INF);

#define flag (i&1^j&1)

    REP_2(i, j, r, c){
        int cap; Sum += RD(cap);
        if (flag) add_edge(s, id(i, j), cap);
        else add_edge(id(i, j, 1), t, cap);
    }
    ...
    REP_3(i, j, d, r, c, 4){
        ...
        if (inGrid(x, y)) add_edge(id(i, j), id(x, y, 1), flag ? G : S);
    }
}
