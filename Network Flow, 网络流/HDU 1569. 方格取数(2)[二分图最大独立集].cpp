...
void init(){
    ...
    REP_2(i, j, r, c){
        int cap; Sum += RD(cap);
        if (i&1^j&1) add_edge(s, id(i, j), cap);
        else add_edge(id(i, j), t, cap);
    }

    REP_2(x, y, r, c) if (x&1^y&1) REP(i, 4){
        int xx = x + dx[i], yy = y + dy[i];
        if (inGrid(xx, yy)) add_edge(id(x, y), id(xx, yy), INF);
    }
}
