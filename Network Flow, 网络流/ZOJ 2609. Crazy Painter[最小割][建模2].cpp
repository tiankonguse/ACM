...
const int R = 30, N = 4*R*R + 9, M = 200009;
...
void init(){
    ...
    FLC(l, u, -1), t = 4*hw + 1, n = t+1, m = 2, fill(hd, hd+n, 0);

    REP_2(i, j, h, w){
        int _j=j; while(j+1<w&&G[i][j+1]==G[i][j]) add_edge(V(0,i,_j),V(1,i,j),INF), ++j;
        add_edge(V(0,i,_j),V(1,i,j),INF), add_edge(s,V(0,i,_j),hh), l[i][_j] = j;
    }

    REP_2(j, i, w, h){
        int _i=i; while(i+1<h&&G[i+1][j]==G[i][j]) add_edge(V(2,i,j),V(3,_i,j),INF), ++i;
        add_edge(V(2,i,j),V(3,_i,j),INF), add_edge(V(3,_i,j),t,vv), u[_i][j] = i;
    }

    REP_2(i, j, h, w) add_edge(V(1,i,j),V(2,i,j),ss);
}
...
void solve(){
    ...
    int cnt = 0; REP_2(i, j, h, w){
        ...
        if(u[i][j]!=-1&&D[V(3,i,j)])++cnt;
        if(D[V(1,i,j)] && !D[V(2,i,j)])++cnt;
    }

    OT(cnt); REP_2(i, j, h, w){
        if(l[i][j]!=-1&&!D[V(0,i,j)])printf("h %d %d %d %d %c\n",i+1,j+1,i+1,l[i][j]+1,G[i][j]);
        if(u[i][j]!=-1&&D[V(3,i,j)])printf("v %d %d %d %d %c\n",i+1,j+1,u[i][j]+1,j+1,G[i][j]);
        if(D[V(1,i,j)] && !D[V(2,i,j)])printf("s %d %d %c\n",i+1,j+1,G[i][j]);
    }
}
