...
const int _N = 109, N = 1109;
int C[N][N], D[N];
int n, s, t, total_flow;
...
bool init(){
    int n, m; if (scanf("%d %d", &n, &m) == EOF) return 0;
    static int p[_N]; REP_1(i, n) RD(p[i]);

    RST(C); total_flow = 0, s = 0, ::n = n; DO(m){
        int u, v; RD(u, v); if (u > v) swap(u, v);
        if (v == n) p[n] += 2;
        else {
            add_edge(::n, u, 2), add_edge(::n, v, 2);
            add_edge(s, ::n++, 2);
        }
    }

    t = ::n++; REP_1(i, n-1){
        int c = p[n]-p[i]-1; if (c < 0) return 1, s = t;
        add_edge(i, t, c);
    }
