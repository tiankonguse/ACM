...
const int N = 5009, M = 2 * (N + 60000) + 9;

int D[N], hd[N], suc[M], to[M], cap[M];
int n, m, s, t; int cut_size; LL tt, max_flow;
...
void Dinitz(){
    ...
    cut_size = 0; fill(D, D+n, 0); dfs();
}


void init(){
    int n, m; RD(n, m); ::m = 2, s = 0, t = n + 1, ::n = t + 1, tt = 0;
    REP_1(i, n){
        int ai; if (RD(ai) > 0) tt += ai, add_edge(s, i, ai);
        else add_edge(i, t, -ai);
    }
    DO(m){
        int x, y; RD(x, y); add_edge(x, y, INF);
    }
    to[0] = s;
}
