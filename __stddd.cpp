...
//}
/* .................................................................................................................................. */
LL cnt[20][20] , dp[20][20] , ten[20] , num[20];
void init(){
    ten[0] = 1;
    for (int i = 1 ; i < 20 ; ++i)
        ten[i] = (ten[i - 1] * 10ll) % MOD;
}
LL dfspre(int i, int s, bool e) {
    if (i==-1) return s == 0;
    if (s < 0) return 0;
    ...
    int u = e?num[i]:9;
    for (int d = 0; d <= u; ++d)
        res += dfspre(i-1, s - (d == 1 || d == 7), e&&d==u);
...
LL dfs(int i , int s , bool e){
    ...
    int u = e ? num[i] : 9;
    for (int d = 0 ; d <= u ; ++d){
        res = (res + mul(ten[i] , mul(d , dfspre(i - 1 , s - (d == 1 || d == 4)  , e && d == u)))) % MOD;
        res = (res + dfs(i - 1 , s - (d == 1 || d == 4) , e && d == u)) % MOD;
    }
...
LL gao(LL x){
    ...
    for (int i = k ; i < 20 ; ++i){
        cout << dfs(s-1, i, 1) << " " << dfspre(s-1, i, 1) << endl;
        res = (res + dfs(s - 1 , i , 1)) % MOD;
    }
    return res;
}
LL L , R;
