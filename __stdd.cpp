...
void dfs(int x, int y, int o, int cc){
    ...
    REP(i, 4) if (i != o){
        ...
        //cout << G[A[x][y]].c[i] << " " << cc << endl;
        //exit(0);

        if (G[A[x][y]].c[i] == cc) dfs(x, y, i, cc);
    }
...
bool ff(){
    RST(filled, vis); cnt = 0;

    //cout << "!!!!!!!!" << endl;

    bool flag = 1;

    /*REP(i, 3) REP(j, 3) REP(o, 4){
        if (!G[A[i][j]].c[o]) cout << i << " " << j << " " << o <<endl;
    }
    cout << endl;*/
    //exit(0);

    REP(i, 3) REP(j, 3) REP(o, 4) if (!vis[i][j][o]){

        //cout << "??" <<endl;

        if (filled[G[A[i][j]].c[o]]){
