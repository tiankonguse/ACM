...
int gao(int k = 0){
    if (k == n) return 0;
        ...
        while (g[sk]){
            ...
            if (sk == t[k]) return gao(k+1);
            if (sk == _sk) return INF;
        }

        int res1 = 0, res2 = 0; int gg[N]; CPY(gg, g);
        ...
        if (sk < t[k]){
            ...
            FOR(i, sk, t[k]){
                if (!g[i]) g[i] = w, res1 += c1;
                else if (g[i]==-w){res1 = INF; break;}
            }

            if (res1 != INF) res1 += gao(k+1); CPY(g, gg);
            if (res1 >= INF) res1 = INF;

            DWN(i, sk, 0){
                if (!g[i]) g[i] = w, res2 += c1;
                else if (g[i]==-w){res2 = INF; break;}
            }
            if (res2 != INF)
            DWN(i, 26, t[k]){
                if (!g[i]) g[i] = w, res2 += c1;
            ...
            if (res2 != INF) res2 += gao(k+1); CPY(g, gg);
...
class NextAndPrev {
public:
	int getMinimum(int c1, int c2, string s, string t) {
	    RST(g), ::c1 = c1, ::c2 = c2, n = SZ(s);

	    REP(i, n) ::s[i] = s[i] - 48, ::t[i] = t[i] - 48;
...
namespace moj_harness {
	...
	int run_test_case(int casenum) {
		...
		case 0: {
			...
			int received__            = NextAndPrev().getMinimum(nextCost, prevCost, start, goal);
