...
const int N = int(3e2) + 9, M = int(1e5) + 9;

int A[N], prd[N], suc[N]; bool vis[N];
Int f[M]; int n, m, mm;
...
int main(){
    ...
    RD(n, mm, m); REP_1(i, n) RD(A[i]); DO(mm){
        int x, y; RD(x, y);
        suc[x] = y, prd[y] = x;
    }
    ...
    REP_1(i, n) if (!prd[i]){
        for (int s=0,x=i;x;x=suc[x])
            s = A[x] += s, vis[x] = 1;
    }

    REP_1(i, n) if (!vis[i]) return 0, puts("0");
    REP_1(i, n) if (suc[i]){
        m -= A[i]; if (m < 0) return 0, puts("0");
    }

    f[0] = 1; REP_1(i, n) FOR_1(j, A[i], m) f[j] += f[j - A[i]];
    OT(f[m]);
}
