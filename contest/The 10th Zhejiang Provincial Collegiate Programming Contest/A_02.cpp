        ...
        for(i=0;i<n;i++){
            ...
            for(j=0;info[i].name[j];j++){
                ...
                while(info[i].pro_num--)
                {
                    scanf("%d",&tmp);
                    double scores=0;
                    if(pro_first[tmp])info[i].score+=2.5,scores = 2.5;
                    else if(pro_second[tmp])info[i].score+=1.5,scores = 1.5;
                    else if(isprime[tmp])info[i].score+=1,scores = 1.0;
                    else info[i].score+=0.3,scores = 0.3;
                }
                for(j=0;j<info[i].contest_num;j++)
                {
                    scanf("%d",&info[i].contest_str[j]);
                }
               // cout<<i<< " $$$$$$$$$$$ "<<info[i].score<<endl;
                if(info[i].contest_num>=3)
                {
                    sort(info[i].contest_str,info[i].contest_str+info[i].contest_num);
                    tmp = info[i].contest_str[2];
                    info[i].score +=  max(0, (tmp - 1200) / 100) * 1.5;
                }
                //cout<<i<< " ****** "<<info[i].score<<endl;
