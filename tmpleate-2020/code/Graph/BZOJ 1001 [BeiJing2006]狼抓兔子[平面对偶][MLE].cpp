...
const int N = int(2e6) + 9, M = 2 * N;

#define id se
vector<pair<DB, int> > adj[N]; PII P[N]; int cap[N], nxt[M], bel[M];
VII adj2[N]; int n, m, s, t;

inline void _add_edge(int x, int y){
    adj[x].PB(MP(atan2(P[y].se-P[x].se, P[y].fi-P[x].fi), m++));
}
inline void add_edgee(int x, int y){
    _add_edge(x, y), _add_edge(y, x);
}
inline void add_edge2(int x, int y, int w){
    adj2[x].PB(MP(y, w));
}

void init(){

    m = 2; int m; RD(n, m); REP(i, n*m) CLR(adj[i]);

    s = 0, t = n*m-1;

    REP(i, n) REP(j, m) P[i*m+j] = MP(i, j);

    REP(i, n) REP(j, m-1){
        RD(cap[::m>>1]);
        add_edgee(i*m+j, i*m+j+1);
    }

    REP(i, n-1) REP(j, m){
        RD(cap[::m>>1]);
        add_edgee(i*m+j, (i+1)*m+j);
    }

    REP(i, n-1) REP(j, m-1){
        RD(cap[::m>>1]);
        add_edgee(i*m+j, (i+1)*m+j+1);
    }
    ...
    n = n*m; REP(i, n){
        SRT(adj[i]); FOR(j, 1, SZ(adj[i])) nxt[adj[i][j-1].id] = adj[i][j].id^1;
        nxt[adj[i].back().id] = adj[i][0].id^1;
    }

    n = 0; FLC(bel, -1); FOR(i, 2, ::m) if (bel[i] == -1){
        bel[i] = n; for (int j=nxt[i];j!=i;j=nxt[j]) bel[j] = n; ++n;
    }


    REP(i, n) CLR(adj2[i]); FOR(i, 2, ::m) add_edge2(bel[i], bel[i^1], cap[i>>1]);
    s = bel[::m-2], t = bel[::m-1];
}
...
int Dijkstra(){
    ...
    CLR(Q); fill(D, D+n, 0x7fffffff); fill(C, C+n, 0); D[s] = 0; Q.push(s);

    int u, v; while (!Q.empty()){
...
// https://gist.github.com/mzry1992/5193875
// http://www.cnblogs.com/swm8023/archive/2012/09/14/2684215.html
