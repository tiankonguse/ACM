...
VII adj[N]; int n, m, s, t, nm;

inline void add_edge(int x, int y){int w; RD(w); adj[x].PB(MP(y, w)), adj[y].PB(MP(x, w));}
inline int id(int i, int j){return i*m+j;} inline int idd(int i, int j){return id(i,j)+nm;}
...
bool init(){
    ...
    if (!nm){
        int res = INF; DO(max(n, m)) checkMin(res, RD()); OT(res);
        return 0;
    }

    REP(i, 2*nm+2) CLR(adj[i]); s = 2*nm, t = s+1;

    REP(j, m) add_edge(s, id(0, j));
    FOR(i, 1, n) REP(j, m) add_edge(idd(i-1, j), id(i, j));
    REP(j, m) add_edge(idd(n-1, j), t);

    REP(i, n){
        add_edge(idd(i, 0), t);
        FOR(j, 1, m) add_edge(id(i, j-1), idd(i, j));
        add_edge(s, id(i, m-1));
    }

    REP_2(i, j, n, m) add_edge(id(i, j), idd(i, j));
...
int Dijkstra(){
    CLR(Q); fill(D, D+n, 0x7fffffff); fill(used, used+n, 0);
    D[s] = 0; Q.push(s); while (!EPT(Q)){
        int u = Q.top(); Q.pop(); if (used[u]) continue;
        if (u == t) break;
        used[u] = 1; ECH(it, adj[u])
            relax(u, it->fi, it->se);
    }
    return D[t];
}
...
/*
const int mask = (1<<21)-1; int cz, op;
int D[N], Q[mask+1]; bool inQ[N];

void relax(int u, int v, int w){
    if (D[u] + w < D[v]){
        D[v] = D[u] + w;
        if(!inQ[v]) inQ[Q[op++] = v] = 1, op &= mask;
    }
}

int SPFA(){
    fill(D, D+n, 0x7fffffff); fill(inQ, inQ+n, 0); cz = 0, op = 1;
    D[Q[0] = s] = 0, inQ[s] = 1; while (cz < op){
        int u = Q[cz++]; cz &= mask;
        inQ[u] = 0; ECH(it, adj[u])
            relax(u, it->fi, it->se);
    }
    return D[t];
}
...
// http://hi.baidu.com/aekdycoin/item/b8fd6adc73c0e71dd78ed0d4
